...
def process_row(row, input_audio_dir, tick_audio_path, beep_audio_path, m_repeat):
    ...
    try:
        ...
        gap_audio = AudioFileClip(tick_audio_path)
        ...
        gap_clip = create_text_clip("...", gap_audio.duration, fontsize=40)

        repeated_text_clip = concatenate_videoclips([jp_text_clip, gap_clip, eng_text_clip] * m_repeat)
        repeated_audio = concatenate_audioclips([jp_audio, gap_audio, eng_audio] * m_repeat)

        beep_audio_clip = AudioFileClip(beep_audio_path)
        beep_clip = create_text_clip("next...", beep_audio_clip.duration, fontsize=40)

        combined_video = concatenate_videoclips([repeated_text_clip, beep_clip])
...
def process_batch(df_batch, input_audio_dir, tick_audio_path, beep_audio_path, output_dir, batch_index, m_repeat, fps):
    """Process a batch of rows to create video and audio files."""
    clips = []
    audios = []
    chapter = "0"
    for _, row in df_batch.iterrows():
        video_clip, audio_clip = process_row(row, input_audio_dir, tick_audio_path, beep_audio_path, m_repeat)
...
def create_videos(input_file, output_dir, input_audio_dir, n_text=5, m_repeat=1, fps=3):
    ...
    for batch_index, start_row in enumerate(range(0, len(df), n_text)):
        ...
        process_batch(df_batch, input_audio_dir, tick_audio_path, beep_audio_path, output_dir, batch_index, m_repeat, fps)
...
if __name__ == "__main__":
    ...
    create_videos(input_file, output_dir, input_audio_dir, n_text=5, m_repeat=3, fps=3)
